...
def main():
  ...
  ####################################################
  # Make Test Predictions and Save to File
  ####################################################
  X_test_bias = dummyAugment(X_test)

  y_pred_test = logistic(X_test_bias @ w) >= 0.5 

  y_pred_test = y_pred_test.astype(int)

  ids = np.arange(len(y_pred_test))

  submission = np.column_stack((ids, y_pred_test))

  with open("test_predictions.csv", "w") as f:
      f.write("id,type\n")  # Header
      np.savetxt(f, submission, fmt="%d,%d", delimiter=",")
...
def calculateNegativeLogLikelihood(X,y,w):
  nll = 0
  smallVal = 1e-15 # to avoid log(0)
  for i in range(len(y)):
    logit = logistic(X[i] @ w)
    nll += (y[i] * np.log(logit + smallVal)) + ((1-y[i]) * np.log(1-logit + smallVal))
